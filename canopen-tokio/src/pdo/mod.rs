...
fn rpdo_communication_params_object(kind: RpdoKind) -> Result<u16, InvalidPdoNumber> {
    let pdo = kind.ord();
...
fn rpdo_mapping_object(kind: RpdoKind) -> Result<u16, InvalidPdoNumber> {
    let pdo = kind.ord();
...
fn tpdo_communication_params_object(kind: TpdoKind) -> Result<u16, InvalidPdoNumber> {
    let pdo = kind.ord();
...
fn tpdo_mapping_object(kind: TpdoKind) -> Result<u16, InvalidPdoNumber> {
    let pdo = kind.ord();
