...
impl CanOpenSocket {
    ...
    pub async fn recv_frame_deadline(
        ...
        tokio::time::timeout_at(deadline.into(), self.socket.recv())
            .await
            .ok()
    }

    /// Send a raw CAN frame.
    pub async fn send_frame(&mut self, frame: &CanFrame) -> std::io::Result<()> {
    ...
    pub async fn sdo_upload<T: sdo::UploadObject>(
        ...
        sdo::sdo_upload(self, node_id, sdo, object, &mut buffer, timeout)
            .await
            .map_err(sdo::UploadError::UploadFailed)?;
        T::parse_buffer(buffer).map_err(sdo::UploadError::ParseFailed)
    }
    ...
    pub async fn read_rpdo_configuration(
        ...
        pdo::read_rpdo_configuration(self, node_id, sdo, kind, timeout).await
    }
    ...
    pub async fn read_tpdo_configuration(
        ...
        pdo::read_tpdo_configuration(self, node_id, sdo, kind, timeout).await
    }
    ...
    pub async fn configure_rpdo(
        ...
        pdo::configure_rpdo(self, node_id, sdo, kind, config, timeout).await
    }
    ...
    pub async fn configure_tpdo(
        ...
        pdo::configure_tpdo(self, node_id, sdo, kind, config, timeout).await
    }
    ...
    pub async fn enable_rpdo(
        ...
        pdo::enable_rpdo(self, node_id, sdo, kind, enable, timeout).await
    }
    ...
    pub async fn enable_tpdo(
        ...
        pdo::enable_tpdo(self, node_id, sdo, kind, enable, timeout).await
    }

    /// Send a SYNC command to the CAN network.
    pub async fn send_sync(&mut self, counter: Option<NonZeroU8>) -> Result<(), std::io::Error> {
    ...
    /// Messages already in the read queue are not returned.
    /// If a message does not match the filter, it is added to the read queue.
    async fn recv_new_by_can_id(
        &mut self,
        can_id: StandardId,
        timeout: Duration,
    ) -> std::io::Result<Option<CanFrame>> {
        self.recv_new_filtered(
            |frame| !frame.is_rtr() && frame.id().to_standard().is_ok_and(|x| x == can_id),
            timeout,
        )
        .await
    }
