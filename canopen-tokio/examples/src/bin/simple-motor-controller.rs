...
async fn do_main(options: Options) -> Result<(), ()> {
    ...
    let socket = CanSocket::bind(&options.interface).map_err(|e| {
        log::error!(
    ...
    socket
        .configure_tpdo(node_id, sdo, tpdo_kind, &tpdo0_config, timeout)
        .await
        .map_err(|e| log::error!("Failed to configure TPDO 0 of node {node_id}: {e}"))?;

    // Configure RPDO 0.
    let rpdo_kind = RpdoKind::First(node_id);
    ...
    socket
        .configure_rpdo(node_id, sdo, rpdo_kind, &rpdo0_config, timeout)
        .await
    ...
    let status: u16 = socket
        .sdo_upload(node_id, sdo, ObjectIndex::new(0x6041, 0), timeout)
        .await
    ...
    socket
        .sdo_download::<u16>(node_id, sdo, ObjectIndex::new(0x6042, 0), 0, timeout)
        .await
        .map_err(|e| log::error!("Failed to set velocity target of node {node_id}: {e}"))?;
    socket
        .sdo_download::<u8>(node_id, sdo, ObjectIndex::new(0x6060, 0), 2, timeout)
        .await
        .map_err(|e| log::error!("Failed to set operation mode to velocity for {node_id}: {e}"))?;
    socket
        .sdo_download::<u16>(node_id, sdo, ObjectIndex::new(0x6040, 0), 0x06, timeout)
        .await
        .map_err(|e| log::error!("Failed to enable voltage to motor of node {node_id}: {e}"))?;
    socket
        .sdo_download::<u16>(node_id, sdo, ObjectIndex::new(0x6040, 0), 0x07, timeout)
        .await
        .map_err(|e| log::error!("Failed to switch on motor of node {node_id}: {e}"))?;
    socket
        .sdo_download::<u16>(node_id, sdo, ObjectIndex::new(0x6040, 0), 0x0F, timeout)
        .await
...
async fn do_step(
    socket: &mut CanOpenSocket,
    velocity_target: u16,
    deadline: Instant,
) -> Result<(), ()> {
    socket
        .send_frame(&CanFrame::new(
            CanId::new(0x201).unwrap(),
            velocity_target.to_le_bytes(),
        ))
        .await
    ...
    log::info!(
        "status: 0x{:02X}, velocity demand: {:5} rpm, motor current: {:5} mA",
        status.status,
        status.velocity_demand,
        status.current_actual as f64
    );
    Ok(())
}
...
fn parse_pdo(input: &CanFrame) -> Result<Status, ()> {
    ...
    if data.len() != 8 {
        log::error!(
            "PDO message has wrong length, expected 8 data bytes, got {}",
            data.len()
        );
        return Err(());
    }
    ...
    T::Error: std::fmt::Display,
{
    ...
    T::try_from(value).map_err(|e| format!("value out of range: {e}"))
}
