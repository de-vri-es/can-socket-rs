...
use canopen_tokio::pdo::{
    PdoMapping, RpdoKind, RpdoTransmissionType, TpdoKind, TpdoTransmissionType,
};
...
async fn do_main(options: Options) -> Result<(), ()> {
    ...
    if let Some(pdo) = options.rpdo {
        let rpdo_kind = RpdoKind::new(options.node_id, pdo).unwrap();

        let mut config = socket
            ...
            .map_err(|e| {
                log::error!(
                    "Failed to read configuration of RPDO {} of node {}: {e}",
                    pdo,
                    options.node_id
                )
            })?;
        ...
        socket
            ...
            .map_err(|e| {
                log::error!(
                    "Failed to configure RPDO {} of node {}: {e}",
                    pdo,
                    options.node_id
                )
            })?;
    } else if let Some(pdo) = options.tpdo {
        let tpdo_kind = TpdoKind::new(options.node_id, pdo).unwrap();
