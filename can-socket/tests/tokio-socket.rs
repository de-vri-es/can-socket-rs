...
use can_socket::{CanData, CanFilter, CanFrame, ExtendedId, StandardId};
...
#[tokio::test]
#[cfg_attr(not(feature = "vcan-tests"), ignore = "enable the \"vcan-tests\" feature to enable this test")]
async fn can_send_rtr() {
	let_assert!(Ok(interface) = TempInterface::new());
	let_assert!(Ok(socket_a) = CanSocket::bind(interface.name()));
	let_assert!(Ok(socket_b) = CanSocket::bind(interface.name()));

	assert!(let Ok(()) = socket_a.send(&CanFrame::new_rtr(2u8)).await);
	let_assert!(Ok(frame) = socket_b.recv().await);
...
#[tokio::test]
#[cfg_attr(not(feature = "vcan-tests"), ignore = "enable the \"vcan-tests\" feature to enable this test")]
async fn enable_recv_own_message() {
	let_assert!(Ok(interface) = TempInterface::new());
	let_assert!(Ok(socket_a) = CanSocket::bind(interface.name()));
...
async fn disable_loopback() {
	...
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(1u8, [1, 2, 3])).await);
	let_assert!(Err(e) = socket_a.recv_timeout(Duration::from_millis(1)).await);
	assert!(e.kind() == std::io::ErrorKind::TimedOut);
}

#[tokio::test]
#[cfg_attr(not(feature = "vcan-tests"), ignore = "enable the \"vcan-tests\" feature to enable this test")]
async fn filter_exact_id() {
	let_assert!(Ok(interface) = TempInterface::new());
	let_assert!(Ok(socket_a) = CanSocket::bind(interface.name()));
	let_assert!(Ok(socket_b) = CanSocket::bind(interface.name()));
	...
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(1u8, [1, 2, 3])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(8u8, [4, 5, 6])).await);
	let_assert!(Ok(frame) = socket_b.recv().await);
	assert!(frame.id().as_u32() == 8);
	assert!(frame.data() == Some(CanData::new([4, 5, 6])));

	let_assert!(Err(e) = socket_b.recv_timeout(Duration::from_millis(1)).await);
	assert!(e.kind() == std::io::ErrorKind::TimedOut);
}

#[tokio::test]
#[cfg_attr(not(feature = "vcan-tests"), ignore = "enable the \"vcan-tests\" feature to enable this test")]
async fn filter_exact_id_rtr_only() {
	let_assert!(Ok(interface) = TempInterface::new());
	let_assert!(Ok(socket_a) = CanSocket::bind(interface.name()));
	let_assert!(Ok(socket_b) = CanSocket::bind(interface.name()));
	...
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(1u8, [1, 2, 3])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(8u8, [4, 5, 6])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new_rtr(8u8)).await);
	let_assert!(Ok(frame) = socket_b.recv().await);
	assert!(frame.id().as_u32() == 8);
	assert!(frame.is_rtr());
...
async fn filter_id_type() {
	...
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(ExtendedId::from(5u8), [1])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(StandardId::from(6), [2])).await);
	let_assert!(Ok(frame) = socket_b.recv().await);
...
async fn filter_mask() {
	...
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(0x1300u16, [1])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(0x1200u16, [2])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(0x12FFu16, [3])).await);
...
async fn multiple_filters() {
	...
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(0x1200u16, [2])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(0x12FFu16, [3])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(0x1900u16, [4])).await);
	assert!(let Ok(()) = socket_a.send(&CanFrame::new(0x2002u16, [5])).await);
